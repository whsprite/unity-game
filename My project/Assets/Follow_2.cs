        ...
        if (Time.time > nextAttackTime)
        {
            ...
            if (sqrMag < sqrAttackRange)
            {
                ...
                //初始化炮弹实例
                 for(int i = 0; i < 12; i++)
                {
                    _firePosition.rotation = Quaternion.Euler(new Vector3(0, 30f * i, 0));
                    GameObject fireInstance = GameObject.Instantiate(firePrefab, _firePosition.position, _firePosition.rotation);
